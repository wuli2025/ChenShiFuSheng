...
/** 点击成品文件 chip → 用系统默认程序打开（HTML 直接进系统浏览器）；
 *  应用文件夹 → 在文件管理器中打开。右侧演示框已移除，统一走外部打开。 */
function openArtifact(path: string) {
  const target = isFolderArtifact(path) ? path.replace(/\/+$/, "") : path;
  invoke("artifact_open_external", { path: target }).catch((e) => {
    toast.error(`打开失败:${humanizeError(e)}`);
  });
}
